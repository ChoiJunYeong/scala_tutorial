...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that : TweetSet) = that union this.left union this.right incl elem
  def mostRetweeted: Tweet = {
    if(remove(elem).descendingByRetweet == Nil)
      elem
    else if(elem.retweets>= remove(elem).mostRetweeted.retweets)
      elem
    else
      remove(elem).mostRetweeted
  }
  def descendingByRetweet: TweetList = {
...
object Main extends App {
  ...
  print(GoogleVsApple.googleTweets.mostRetweeted)
