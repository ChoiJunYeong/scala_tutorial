...
object FunSets {
   ...
   */
    def singletonSet(elem: Int): Set = test => elem==test
   ...
   */
    def filter(s: Set, p: Int => Boolean): Set = elem => contains(s,elem) && contains(p,elem)
   ...
   * that satisfies `p`.
   */
    def exists(s: Set, p: Int => Boolean): Boolean = if(forall(s,elem => if (p(elem)) false else true) == true) false else true
